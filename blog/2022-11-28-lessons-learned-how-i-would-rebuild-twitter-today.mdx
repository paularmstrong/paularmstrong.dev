...
I would look at using [Deno](deno.land) instead of [Node.js](https://nodejs.org) with a big up-front evaluation. There are a lot of unknowns in running a new service with a new runtime, so it would be really important to feel confident using one vs another.
