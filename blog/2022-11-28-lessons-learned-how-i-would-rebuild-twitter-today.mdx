...
pubDate: 11/28/2022, 9:00:00 AM GMT-0700
...
With [Solid-js](https://www.solidjs.com/)’ [signals](https://www.solidjs.com/tutorial/introduction_signals) (now available in [Preact](https://preactjs.com/blog/introducing-signals/) as well) that only allow updating the minimal set of dependents, the above would not have been difficult.
...
We used [React Native for Web](https://necolas.github.io/react-native-web/) (<acronym title="React Native for Web">RNW</acronym>) and I was really bullish on its benefits for years. In fact, its creator was our tech lead for my first few years there until he left to join the React core team. While we got a lot out of <acronym title="React Native for Web">RNW</acronym>, we didn’t get any benefit out of the core selling point: cross-platform code. The native apps are highly optimized, do not, and will not use React Native. Without going into too much detail, I’ll just say it is not a battle to even try to fight.
...
Most CSS-in-JS solutions do not create separate CSS assets that are cached in the CDN. All of the styles are delivered within the JS, causing extra network, evaluation, and runtime contention within the application. For our performance-first approach, it is imperative that we make gains here.
...
The Webpack configuration’s at scale become unruly beasts. There were at most a handful of engineers at Twitter (more likely one or two) that were confident in making changes to the configuration.
...
I would look at using [Deno](deno.land) instead of [Node.js](https://nodejs.org) this into a big up-front evaluation. There are a lot of unknowns in running a new service with a new runtime, so it would be really important to feel confident using one vs another.
