...
```bash npm2yarn
npm install --save typescript preact preact-router
npm install --save-dev autoprefixer parcel postcss tailwindcss
```

Next we need an entrypoint HTML file for Parcel to pick up as well as the CSS and Preact application code references.
