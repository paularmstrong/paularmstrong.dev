...
The source of all custom commands are written in the same language as the rest of your repository and each command is represented by a distinct file, easy to find within the [`commands` directory](https://onerepo.tools/docs/config/#commandsdirectory). This should enable everyone on your team that works with JavaScript to be able to contribute and learn the tooling without always needing to interrupt a specialist.
